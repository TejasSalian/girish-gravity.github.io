...
$(document).ready(function () {
    ...
    blockUI("body");
    _gUserService.GetProjectList($("#hdnOrgName").val(), cb, true);
        ...
        for (var i = 0 ; i < res.length; i++) {
            ...
            htm += "<tr>" +
                          "<td id='" + res[i].projectid + "' stage='" + res[i].projectstage + "' onclick='onClickOpenProject(this)'>" + res[i].projectname + "<br />" +
                              "<span>Started on " + new Date(res[i].startdate).toLocaleDateString() + "</span></td>" +
                            "<td>" + res[i].projecttypename + "</td>" +
                            "<td>" + res[i].firstname + " " + res[i].lastname + "<br />" +
...
function onLoadCollabDashboard() {
    var data = JSON.parse($("#hdnUsers").val());
    var html = "";
    $("#noAssigned").html(data.length);
    for (var index = 0; index < data.length; index++) {
        html += '<div class="col-md-12">';
        if (data[index].isverified)
            html += generateApprovedRequest(false, data[index]);
        else
            html += generatePendingRequest(false, data[index]);

        html += "</div>";
    }

    for (var i = 0; i < 5 - data.length; i++) {
        html += generateAddCollab();
    }
...
function generateAddCollab() {
    var html = '<div class="col-md-12 cbr-box">' +
                    '<div class="add-coll-box">' +
                        '<div class="add-new-colb" style="display: block"  onclick="add_new_collob($(this).find(&apos;img&apos;))">' +
                            '<span class="addc-btn">' +
                                '<img src="assets/imgs/add-coll.png" class="img-w25" onclick="add_new_collob(this)" />' +
                            '</span>' +
                            '<div class="addcbc1">' +
                                ' <span class="lr14b clr-blue" onclick="add_new_collob(this)">Add Collaborator' +
                                '</span>' +
                            '</div>' +
                        ' </div>' +
                        '<div class="close-colb" style="display: none">' +
                            '<span  class="addc-btn">' +
                                '<img src="assets/imgs/close-coll.png"class="img-w25" onclick="close_new_collob(this)" />' +
                            '</span>' +
                            '<div class="addcbc1" >' +
                                    '<input type="text" id="colb-name" placeholder="Name" style="padding-left: 15px" />' +
                                '<input type="email" id="colb-email" placeholder="E-mail address" style="padding-left: 15px;     margin-top: 10px;" />' +
                            '</div>' +
                            '<div  class="addcbc2">' +
                                '<input type="button" style="width: 180px;"  class="btn btn-blue" onclick="onClickAddCollab(this)" value="Add Collaborator" />' +
...
function onClickAddCollab(ele) {
    var ab = new alertbox();
    ab.id = "#collAlert";

    var parent = $(ele).parents(".cbr-box");
    var ud = new User();
    ud.organization = $("#hdnOrgName").val();
    ud.firstname = $(parent).find("#colb-name").val();
    ud.email = $(parent).find("#colb-email").val();
    ud.regdate = new Date().toLocaleString();


    if (ud.firstname == "") {
        ab.showError('Please enter collaborator name.');
        return;
    }

    if (ud.email == "") {
        ab.showError('Please enter collaborator email.');
        return;
    }
    ...
    var html = generatePendingRequest(false, ud);

    var cb = new CallBack();
    cb.func = "onInvitedUser"
    cb.data = ab;

    _gUserService.InviteCollabrator(ud, cb, true);
...
Note.prototype.updateServer = function () {
    ...
    _gUserService.UpdateComment(pD, cb, true);
    blockUI("body");
}
...
function CollaboratorProjectDetail(ele) {
    ...
    if (pData != null && pData.length > 0) {
        ...
        GetColProjectDetails(projectId, pData[0].projectstage, ele, true);
...
function GetColProjectDetails(projectId, stage, element, isFirstLoad) {
    ...
    var data = '<div style="height: 100%;">' +
                '<ul class="nav nav-tabs container" role="tablist">' +
                    ...
                    'onclick="GetColProjectDetails(' + projectId + ', 0, this, false)">' +
                    ...
                    'onclick="GetColProjectDetails(' + projectId + ', 1, this, false)">' +
                    ...
                    'onclick="GetColProjectDetails(' + projectId + ', 2, this, false)">' +
                    ...
                    'onclick="GetColProjectDetails(' + projectId + ', 3, this, false)">' +
...
function generateVersion(versionData, projectId, stage) {
    ...
    if (versionData == null || versionData.versiondata[0] == undefined) {
        if (stage == 0) {
            $('#infoArch').html('<div class="nVersion"> No version has been uploaded yet </div>');
        }
        else if (stage == 1) {
            $('#infoArch').html('<div class="nVersion"> No version has been uploaded yet </div>');
        }
        else if (stage == 2) {
            $('#storyBoard').html('<div class="nVersion"> No version has been uploaded yet </div>');
        }
        else if (stage == 3) {
            $('#finalArt').html('<div class="nVersion"> No version has been uploaded yet </div>');
        }
    ...
    if (latestVersion != null) {
        ...
        //var imgdata = latestVersionImg.split(";");
        //var exten = imgdata[0].split("/");
        //imageExt = exten[1];
    ...
    var data = '<div class="container" style="padding-top: 40px;">' +
                    ...
                    '</div>' +
                    '<div class="col-md-12">' +
                        '<button ' + disabled + ' type="button" style="width: 120px;" onclick="approveVersion(' + projectId + ', ' + pData[0].projectstage + ', ' + p.versionid + ')" ' +
                        'class="btn-blue btn pull-right">' + isApproved + '</button>' +
                        vHtml +
                    '</div>' +
                    ...
                    '<div class="imgTool_content">'
                        ...
                        '<div id="imgPort"  class="imgdiv">' +
                            '<img id="image" src="service/getfile.aspx?type=vimage&id=' + p.versionid + '" style="display: none; width: auto; height: 100%;" />' +
                            '<div class="imgTool">' +
                                '<table class="fullT">' +
                                    '<tr>' +
                                        '<td>' +
                                       ...
                                       '<td><a  onclick="if(!$(this).hasClass(&apos;disabled&apos;)){addComment();}" class="' + dis + '">' +
                                        ...
                                        '<td></td>' +
                                        '<td>' +
                                            '<a href="service/getfile.aspx?type=vimage&id=' + p.versionid + '" target="_blank" ><img src="assets/imgs/save.png" />' +
                                            '<p>Save</p></a>' +
                                        '</td>' +
                                        '<td onclick="onClickSendFeedback()">' +
                                            '<img src="assets/imgs/send_feedback.png" />' +
                                            '<p> Send Feedback </p>' +
                                        '</td>' +
                                    '</tr>' +
                                '</table>' +
...
function onClickStartProject(ele) {
    ...
    var cb = new CallBack();
    cb.func = "onClickedStartProject";
   

    _gUserService.GetProjectTemplates(new ProjectDetail(), cb, true);

    blockUI("body");
}

function onClickedStartProject(res, cb) {
    var data = res;
            ...
            layout += '<div class="layouts" id="lay' + data[index].templateid + '">' +
                        '<div style="width: 420px; margin: 0 auto;">' +
                            '<div class="lay' + data[index].templateid + '">' +
                                '<img src="' + data[index].templateimage + '" />' +
                            '</div>' +
                         '</div>' +
...
function approveVersion(projectId, stage, versionId) {
    ...
    if (stage == 3) {
        ...
        _gUserService.ApproveProject(data, null, false);
    }
    else {
        data.stage = (stage + 1);
        _gUserService.ChangeProjectStage(data, null, false);
    }
