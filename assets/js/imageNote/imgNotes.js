...
//					}
				...
				$("#_"+$elem.attr("id")).parent().find(".saveImg").show();
				$("#_" + $elem.attr("id")).parent().find(".editImg").hide();
			},
/*
...
//				var st = "style='position:absolute;top:" + pos.top + "px; left:" + (pos.left + width) + "px'";

			    //				return "<div " + st + "> " + $elem.data("note") + " </div>"

			    //$('[data-toggle="popover"]').popover({
			    //    html: true,
			    //    content: function () {
			    //        return $(this).find(".pCont").html();
			    //    }
			    //});

				//$('[data-toggle="popover"]').not($elem.find('[data-toggle="popover"]')).popover('hide');
				//$elem.find('[data-toggle="popover"]').popover('show');
...
//		attach the imgViewer plugin for zooming and panning with a custon click and update callbacks
			$img.imgViewer({
			    appendto:".imgdiv",
							...
							extra: "<img class='zIn' src='assets/imgs/zoom_in.png' onclick='zoomIn()' /><img class='zOut' src='assets/imgs/zoom_out.png' onclick='zoomOut()' />"
			});
			$img.imgViewer("option", "onClick", function (ev, imgv) {
			    if (self.options.isAdd) {
			        ...
			        var innerImg = $("#imgPort").find(".viewport").find("img");
			        var MainIng = innerImg[0];
			        var MainImgP = $(MainIng).offset();

			        var extrax = MainImgP.top * self.options.zoom - MainImgP.top;
			        var extray = MainImgP.left * self.options.zoom - MainImgP.left;
			        ...
			        var rpos = imgv.cursorToImg(ev.pageX , ev.pageY);
			    					...
			    					self.options.isAdd = false;
			    				}
			});

			$img.imgViewer("option", "onUpdate", function (ev, imgv) {
 ...
 *	Add a note
 */
		addNote: function(relx, rely, text,id,uid,uname) {
			var self = this;
			this.noteCount++;
			var elem = this.options.onAdd.call(this);
			var $elem = $(elem);
			$elem.attr("class", "ppOver");
			$elem.attr("id", "pop" +this.noteCount);
			...
			$elem.attr("data-relx", relx);
			$elem.attr("data-rely", rely);
			$elem.attr("data-id", id);
			$elem.attr("data-uid", uid);
			$elem.attr("data-uname", uname);
			if (id == "" || id == undefined)
			    id = 0;

			$elem.append("<input type='hidden' class='nID' value='" + id + "' /><div class='pCont'> " + text + " </div>");
 ...
 */
		import: function(notes) {
			var self = this;
			$.each(notes, function() {
				self.addNote(this.x, this.y, this.note,this.id, this.userid, this.username);
			});
			$(this.img).imgViewer("update");
		},
/*
