...
; (function ($) {
    $.widget("wgm.imgViewer", {
        ...
        _create: function () {
             ...
             */
            self.zimg = $("<img/>", { "src": self.img.src }).appendTo(self.options.appendto).wrap("<div  class='viewport'/>");
            var $zimg = $(self.zimg);

            //		the container or viewport for the image view
            self.view = $(self.zimg).parent();
            var $view = $(self.view);
            $view.append(self.options.extra);
            ...
            if (window.navigator.msPointerEnabled) {
                ...
                $zimg.on("transformstart", function (ev) {
                    ...
                    if (self.options.zoomable) {
                        ...
                        self.pinchstart = {
                            x: (ev.start.point[0].x + ev.start.point[1].x) / 2,
                ...
                $zimg.on("mousedown", function (e) {
                    function endDrag(e) {
                        //setTimeout(function () { self.dragging = false; }, 100);
                        e.preventDefault();
                        stopRenderLoop();
                        $zimg.off("mousemove");
                        $zimg.off("mouseup");
        ...
        cursorToImg: function (cx, cy) {
            if (this.ready) {
                var $img = $($("#imgPort").find(".viewport").find("img")[0]),
					width = $img.width(),
					height = $img.height(),
					offset = $("#imgPort").find(".viewport").offset();
                var zLeft = width / 2 - this.vCenter.x * this.options.zoom;
                var zTop = height / 2 - this.vCenter.y * this.options.zoom;
        ...
        update: function () {
            if (this.ready) {
                ...
                $(this.zimg).attr("data-x", xt);
                $(this.zimg).attr("data-y", yt);

                $(this.zimg).css({
                    transform: "translate(" + xt + "px," + yt + "px) scale(" + zoom + "," + zoom + ")",
                    transition: "0.3s ease transform", cursor: 'move', WebkitTransition: '0.3s ease transform',
                    MozTransition: '0.3s ease transform',
                    MsTransition: '0.3s ease transform',
                    OTransition: '0.3s ease transform'
                });
                /*
